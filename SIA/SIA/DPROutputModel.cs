...
namespace SIA
{
    public class DPROutputModel : INotifyPropertyChanged
    {
        ...
        private ObservableCollection<DPROutputModel> _tmpCollection;
        ...
        public async Task GetDPROutput(string DPRID)
        {
            try
            {
                var Url = new Uri("http://172.11.66.181/xampp/SIAGetDPROutput.php?dprid=" + DPRID);
                ...
                {
                    List<DPROutputModel> tmpModel = await Task.Run(() => JsonConvert.DeserializeObject<List<DPROutputModel>>(contents));
                    _tmpCollection = new ObservableCollection<DPROutputModel>();
        ...
        public ObservableCollection<DPROutputModel> TmpCollection
        {
            get
            {
        ...
        public async Task<string> PostOutput(string barcode, string fname, string lotNo, string DPRID, string JORNO, int last = 0)
        {
            //used for output posting to sqlite
            //postResult = await i.PostOutput(i.barCode, mstId.Text, i.lotNumber, lblDPRid.Text.Trim(),1);
            try
            {
                var Url = "";
                if (last == 0)
                {
                    Url = "http://172.11.66.181/xampp/siasaveoutput.php?barcode=" + barcode + "&fname=" + fname + "&lotno=" + lotNo + "&DPRID=" + DPRID;
                }
                else
                {
                    Url = "http://172.11.66.181/xampp/siasaveoutput.php?barcode=" + barcode + "&fname=" + fname + "&lotno=" + lotNo + "&DPRID=" + DPRID + "&last=1";                    
                }
        ...
        public async Task<string> PostOutputCSV(string barcode, string fname, string DPRID, string JORNO, int outputID, int last = 0)
        {
            //used for output posting to CSV            
            //http://172.11.66.181/xampp/siasaveoutputCSV.php?barcode=" + barcode + "&fname=" + fname + "&outputID=" + outputID.ToString().Trim() + "&DPRID=" + DPRID + "&jorno=" + JORNO + "&last=1"
            //http://172.11.66.181/xampp/siasaveoutputCSV.php?barcode=" + barcode + "&fname=" + fname + "&outputID=" + outputID.ToString().Trim() + "&DPRID=" + DPRID + "&jorno=" + JORNO
            try
            {
                var Url = "";
                if (last == 0)
                {
                    Url = "http://172.11.66.181/xampp/siasaveoutputCSV.php?barcode=" + barcode + "&fname=" + fname + "&outputID=" + outputID.ToString().Trim() + "&DPRID=" + DPRID + "&jorno="+ JORNO;
                }
                else
                {
                    Url = "http://172.11.66.181/xampp/siasaveoutputCSV.php?barcode=" + barcode + "&fname=" + fname + "&outputID=" + outputID.ToString().Trim() + "&DPRID=" + DPRID + "&jorno=" + JORNO + "&last=1";
                }
        ...
        public async Task<string> PostConsumption(string inBarcode, string fName, int last = 0)
        {
            ...
            try
            {
                ...
                if (last == 0)
                {
                    Url = "http://172.11.66.181/xampp/siapostconsumption.php?barcode=" + inBarcode + "&fname=" + fName;
                }
                else
                {
                    Url = "http://172.11.66.181/xampp/siapostconsumption.php?barcode=" + inBarcode + "&fname=" + fName + "&last=1";
                }
        ...
        public async Task<string> PostDetailsShipment(string inBarcode, string fName, int last=0)
        {
            //used for shipment posting to csv
            try
            {
                var Url = "";
                if (last == 0)
                {
                    Url = "http://172.11.66.181/xampp/siapostshipment.php?barcode=" + inBarcode + "&fname=" + fName;
                }
                else
                {
                    Url = "http://172.11.66.181/xampp/siapostshipment.php?barcode=" + inBarcode + "&fname=" + fName + "&last=1";
                }
