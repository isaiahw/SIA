                ...
                {                                        
                    ...
                    if(IsConnected)
                    {
                        ...
                        foreach (ScannedItem i in scannedItems)
                        {
                            if (y == x)
                            {
                                postResult = await i.PostDetails(i.barCode, mstId.Text, 1);
                            }
                            else
                            {
                                postResult = await i.PostDetails(i.barCode, mstId.Text);
                            }
                            //postResult = await i.PostDetails(i.barCode, mstId.Text); //13Dec2016 why post 2 times???
